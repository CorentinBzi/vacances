  ...
  return (
    <div ref={boxRef} className={cn("relative", className)}>
      ...
      {open && results.length > 0 && (
        <ul className="absolute z-40 mt-2 max-h-72 w-full overflow-auto rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xl">
          {results.map((r) => (
            <li key={r.id}>
              <button
                type="button"
                ...
                className="flex w-full items-start gap-2 rounded-xl px-3 py-2 text-left hover:bg-lagoon-50"
              >
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-lagoon-500" />
                <span>
